...
public void OnPluginStart()
{
	...
	g_Cvar_ModsMaster = CreateConVar("sm_mapvote_mods_master", "1", "Will voting for the custom mode?", _, true, 0.0, true, 1.0);
...
void InitiateVote(MapChange when, ArrayList inputlist=null)
{
	...
	if(!g_MapVoteCompleted && g_HasVoteStarted && !CF_EnabledThisMoment() && g_Cvar_ModsMaster.BoolValue) CreateTimer(1.0, TimerStartModeVote, _, TIMER_REPEAT|TIMER_FLAG_NO_MAPCHANGE);
}

public Action TimerStartModeVote(Handle timer) 
{
	...
	{
		if(sMapName[0]) MM_StartVote(1, sMapName);
